...
void CreatureObjectImplementation::activatePassiveWoundRegeneration() {
	...
	if(healthRegen > 0) {
		...
		if(healthWoundHeal >= 100) {
			...
			//HEALTH BUFFS
			if(autoDoctor == true && healthWoundTotal == 0  && actionWoundTotal == 0 && isSitting()){
	...
	if(actionRegen > 0) {
		...
		if(actionWoundHeal >= 100) {
			...
			//BUFFS
			if(autoDoctor == true && actionWoundTotal == 0 && healthWoundTotal == 0 && isSitting()){
				ZoneServer* zoneServer = getZoneServer(); //Ethan edit 6-7-24 (SINGLE PLAYER ENTERTAINER)
	...
	if(medicalWoundChargeTotal > 0){
		asCreatureObject()->subtractCashCredits(healPrice * medicalWoundChargeTotal);
	...
	if(mindRegen > 0) {
		...
		if(mindWoundHeal >= 100) {
			...
			if(autoEntertainer == true && cash > (healPrice * mindWoundTotal) && isSitting()){
				...
				if (mindWoundTotal > 0){
					asCreatureObject()->subtractCashCredits(healPrice * mindWoundTotal);
