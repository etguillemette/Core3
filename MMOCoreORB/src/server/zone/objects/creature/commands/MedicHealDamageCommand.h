...
#ifndef MEDICHEALDAMAGECOMMAND_H_
#define MEDICHEALDAMAGECOMMAND_H_
...
class MedicHealDamageCommand : public QueueCommand {
	float range;
	float mindCost;

public:
	MedicHealDamageCommand(const String& name, ZoneProcessServer* server)
	...
	bool canPerformSkill(CreatureObject* creature, CreatureObject* creatureTarget, int mindCostNew) const {
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!canPerformSkill(creature, targetCreature, mindCostNew))
			return GENERALERROR;
		...
		int healthHealed = 100, actionHealed = 100, mindHealed = 0;
		bool notifyObservers = true;
        bool isArea = true;
        int areaRange = 10.0;
        bool isRanged = true;
		...
		if (isArea) {
			handleArea(creature, targetCreature, stimPack, areaRange);
		}
...
#endif //MEDICHEALDAMAGECOMMAND_H_
