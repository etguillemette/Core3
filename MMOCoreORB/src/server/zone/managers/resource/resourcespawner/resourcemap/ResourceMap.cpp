...
void ResourceMap::addToSuiListBox(SuiListBox* suil, const String& name) {
	...
	for(int i = 0; i < spawns.size(); ++i){
		if(spawns.get(i)->inShift() || spawns.get(i)->recentShift(90) || true){ //Ethan edit 6-3-24 (RESOURCE VENDOR) --Edit 1-24-25 ... Added the "|| true"... I want to list everything now, even archival resources
...
bool ResourceMap::hasSpawns(const String& name) {
	...
	for(int i = 0; i < spawns.size(); ++i){
		if(spawns.get(i)->recentShift(90)){ //Ethan edit 6-3-24 (RESOURCE VENDOR)
			hasRecent = true;
			break;
		} //Ethan edit 6-3-24 (RESOURCE VENDOR)
	}
