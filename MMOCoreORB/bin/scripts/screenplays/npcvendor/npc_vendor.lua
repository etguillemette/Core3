...
function NPCVendor:getWaresTable(category)
	...
	elseif category == "hires_tank" then
	...
	if(templatePath ~= nil) then
		for i = 1, quantity do
			...
			if(newSerial ~= nil) then
				...
				print(junkValue)
				tano:setJunkValue(junkValue) --Ethan edit 12-3-24 (SLICING REVAMP) (NPC VENDOR) - Makes it so that you can sell back used gear for half its cost, OR slice them to increase their value.
				tano:setJunkDealerNeeded(1) -- Ethan edit 12-5-24 (SLICING REVAMP) (NPC VENDOR) - Makes it so that you can sell back used gear for half its cost, OR slice them to increase their value.
...
function NPCVendor:sendResourceSaleSui(pNpc, pPlayer, screenID)
	...
	suiManager:sendResourceListBox(pPlayer,"@veteran:resource_title","@veteran:choose_class","@cancel","@back","@ok","NPCVendor","ResourcePurchaseSuiCallback",32)
	--suiManager:sendListBox(pNpc, pPlayer, "@event_perk:pro_show_list_title", "@event_perk:pro_show_list_desc", 2, "@cancel", "", "@ok", "NPCVendor", "handleSuiPurchase", 32, options)
